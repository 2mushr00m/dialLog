...
//        return "ko-KR";
        ...
        try {
            ...
            int sampleRateHz = extractSampleRateHz(resolved.file);
            if (snippet.length == 0) {
    ...
    private String requestSnippetTranscript(@NonNull byte[] snippet, @Nullable String mimeType, int sampleRateHz) throws Exception {
        ...
        applyAudioEncoding(request.config, mimeType, sampleRateHz);
        ...
        GoogleSttResponse.Result first = body.results.get(0);
        if (first.alternatives == null || first.alternatives.isEmpty()) {
            return "";
        }
        GoogleSttResponse.Alternative alt = first.alternatives.get(0);
        return alt.transcript != null ? alt.transcript : "";
    }

    private void applyAudioEncoding(@NonNull GoogleSttRequest.Config config,
                                    @Nullable String mimeType,
                                    int sampleRateHz) {
        EncodingAttributes attrs = inferEncodingAttributes(mimeType);
        if (attrs == null) {
            if (!TextUtils.isEmpty(mimeType)) {
                Log.d(TAG, "applyAudioEncoding: unsupported mimeType=" + mimeType);
            }
            return;
        }
        ...
        if (attrs.requiresSampleRate) {
            ...
            if (sampleRateHz <= 0) {
                Log.d(TAG, "applyAudioEncoding: using fallback sample rate for mimeType=" + mimeType);
            }
            config.sampleRateHertz = effectiveRate;
        }
    }

    private EncodingAttributes inferEncodingAttributes(@Nullable String mimeType) {
        if (TextUtils.isEmpty(mimeType)) {
            return null;
        }
    ...
    private int extractSampleRateHz(@NonNull File file) {
        MediaMetadataRetriever retriever = new MediaMetadataRetriever();
        try {
            retriever.setDataSource(file.getAbsolutePath());
